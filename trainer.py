...
class MyEngine(Engine):
    ...
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        def attach_running_average(engine, metric_name):
            RunningAverage(output_transform=lambda x: x[metric_name]).attach( # Attaches current metric to provided engine. On the end of engine’s run, engine.state.metrics dictionary will contain computed metric’s value under provided name.
                engine,
                metric_name,
            )
            # output_transform (Optional[Callable]) 
            # – a function to use to transform the output if src is None and corresponds the output of process function. 
